...
/*SCOTT에 응용*/
SELECT E.ENAME 사원명, E.DEPTNO, AVG(E.SAL) OVER (PARTITION BY DEPTNO)
FROM scott.EMP E;
...
SELECT E.ENAME                                             사원명
     , E.DEPTNO
     , AVG(E.SAL) OVER (PARTITION BY DEPTNO)               부서별평균급여
     , MAX(E.SAL) OVER (PARTITION BY DEPTNO)               최대
...
SELECT E.ENAME                                             사원명
     ...
     ,FIRST_VALUE(E.JOB) over (PARTITION BY E.DEPTNO ORDER BY E.SAL)
FROM scott.EMP E;
